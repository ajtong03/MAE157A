...
import trajectory
from dynamics import dynamics  # needed for quat_to_rot
...
t0 = 0.0
state[0:3] = [trajectory.x_t(t0), trajectory.y_t(t0), trajectory.z_t(t0)]
state[3:6] = [trajectory.vx_t(t0), trajectory.vy_t(t0), trajectory.vz_t(t0)]
state[6:10] = euler_to_quat(0, 0, 0)
state[10:13] = np.zeros(3)

# prepare plot
global history, axis_arrows
fig = plt.figure()
...
# draw gate at z=1 rotated 90° about Y
...
def update(i):
    ...
    # kinematic update
    state[0:3] = pd; state[3:6] = vd
    state[6:10] = attitude_from_acc(ad + np.array([0,0,9.81]), 0); state[10:13] = np.zeros(3)
    ...
    for dx,dy,dz in offs:
        Xw = cos(ps)*cos(th)*dx + (cos(ps)*sin(phi)*sin(th)-cos(phi)*sin(ps))*dy + (cos(phi)*cos(ps)*sin(th)+sin(phi)*sin(ps))*dz + pd[0]
        Yw = sin(ps)*cos(th)*dx + (sin(ps)*sin(phi)*sin(th)+cos(phi)*cos(ps))*dy + (cos(phi)*sin(ps)*sin(th)-cos(ps)*sin(phi))*dz + pd[1]
        Zw = -sin(th)*dx + cos(th)*sin(phi)*dy + cos(phi)*cos(th)*dz + pd[2]
    ...
    # remove old axes arrows
    for art in axis_arrows:
        art.remove()
    axis_arrows.clear()
    # draw new orientation axes arrows using dyn.quat_to_rot
    pos = pd
    R = dyn.quat_to_rot(state[6:10])
    axis_arrows.append(ax.quiver(pos[0],pos[1],pos[2], R[0,0],R[1,0],R[2,0], length=0.3, normalize=True, color='r'))
    axis_arrows.append(ax.quiver(pos[0],pos[1],pos[2], R[0,1],R[1,1],R[2,1], length=0.3, normalize=True, color='g'))
    axis_arrows.append(ax.quiver(pos[0],pos[1],pos[2], R[0,2],R[1,2],R[2,2], length=0.3, normalize=True, color='b'))
